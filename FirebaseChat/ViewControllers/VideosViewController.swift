...
class VideosViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        self.documentsURL = URL.createFolder(folderName: "Videos")
        self.videos = documentsURL?.listFilesFromDownloadsFolder() ?? []
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if self.push {
            ...
            if let cell = self.collectionView.cellForItem(at: self.selectedIndexPath) {
                let localURL = documentsURL!.appendingPathComponent(self.videos[self.selectedIndexPath.row])
                let imageView = cell.viewWithTag(1) as? UIImageView
                imageView?.image = URL.init(string: "file://" + localURL.path)!.generateThumbnail(original: false)?.square()
            }
        }else {
    ...
    @objc func deleteAction(_ notification: NSNotification) {
        ...
        for i in self.check {
            let fileName = self.videos[i.row]
            self.videos.remove(at: i.row)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let localURL = documentsURL!.appendingPathComponent(self.videos[indexPath.row])
        imageView?.image = URL.init(string: "file://" + localURL.path)!.generateThumbnail(original: false)?.square()
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if self.check.isEmpty {
            ...
            if let cell = self.collectionView.cellForItem(at: indexPath) {
                let localURL = documentsURL!.appendingPathComponent(self.videos[indexPath.row])
                let imageView = cell.viewWithTag(1) as? UIImageView
                imageView?.image = URL.init(string: "file://" + localURL.path)!.generateThumbnail(original: true)
            }
            CustomDelegate.shared.previewVideo(collectionView: self.collectionView, selectedIndexPath: selectedIndexPath, video: self.videos[indexPath.row])
        }else {
            if let index = self.check.firstIndex(of: indexPath) {
                self.check.remove(at: index)
            }else{
